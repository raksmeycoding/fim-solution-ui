...
import { useRegisterMutation } from "../../../redux/app/services/authApi";
import {
  AuthRegisterRequest,
  GenericDtoBody,
  GenericDtoResult,
} from "../../../types";
import { INIT_GENERIC_DTO_AUTH_REGISTER_REQUEST } from "../../../utils/init_data";
// import { ServerError } from "../../../exception/ServerError";
import toast, { Toaster } from "react-hot-toast";
import { useSelector } from "react-redux";
import {
  removeIsRegister,
  selectSuccessRegister,
...
interface ServerError extends GenericDto<null> {} // Assuming data is null for errors
...
function SignUP() {
  ...
  const isRegisterSuccess = useSelector(selectSuccessRegister);
  const dispatch = useDispatch();
  const [register, { isLoading, isSuccess, isError }] = useRegisterMutation();
  const [errorMsg, setErrorMessage] = useState<String>("");

  const [genericDtoBody, setGenericDtoBody] = useState<
    GenericDtoBody<AuthRegisterRequest>
  >(INIT_GENERIC_DTO_AUTH_REGISTER_REQUEST);

  // Handle form data change
  const handleChange = ({
    target: { name, value },
  }: React.ChangeEvent<HTMLInputElement>) => {
    setGenericDtoBody((pre) => {
      return { ...pre, data: { ...pre.data, [name]: value } };
    });
  };
  ...
  const handleLogin = async () => {
    ...
    try {
      const result = await register(genericDtoBody).unwrap();
      // in the try blog automatically success
      if (result.code === "201") {
        navigate("/login");
      }
    } catch (error: any) {
      errorNotify("Login failed, check logs to see error message");
      // // Determine the error type
      // const serverError = error as GenericDto<null>;
      // // Handle the error based on its structure
      // const errorMessage = serverError.message || "An unknown error occurred";
      // console.log("Error code:", serverError?.data?.message);
      console.log("Error message:", error);
      setErrorMessage(error?.data?.message);
    } finally {
      // rest auth register to default value
      setGenericDtoBody(INIT_GENERIC_DTO_AUTH_REGISTER_REQUEST);
    }
  };

  return (
    <>
      <div className=" bg-gradient-to-tl from-[#d9ffde] h-full pb-[200px] to-[#28ffff] w-full py-16 px-4">
          ...
          <div className="bg-white shadow rounded lg:w-1/3  md:w-1/2 w-full p-10 mt-4">
            ...
            <div className="flex flex-row justify-between items-center gap-2 md:gap-4">
              <div className="fistName-section w-full">
                ...
                <input
                  value={genericDtoBody.data["firstName"]}
              ...
              <div className="lastName-section w-full">
                ...
                <input
                  value={genericDtoBody.data["lastName"]}
                  onChange={handleChange}
                  name="lastName"
                  aria-labelledby="email"
                  type="email"
            ...
            <div className="mt-4  w-full">
              ...
              <div className="relative flex items-center justify-center">
                <input
                  value={genericDtoBody.data["password"]}
                  onChange={handleChange}
                  name="password"
                  id="pass"
                  type="password"
            ...
            <div className="mt-4  w-full">
              ...
              <div className="relative flex items-center justify-center">
                <input
                  value={genericDtoBody.data["confirmPassword"]}
                  onChange={handleChange}
