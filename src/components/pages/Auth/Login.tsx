...
import {
  useRegisterMutation,
  useLoginMutation,
} from "../../../redux/app/services/authApi";
import {
  GenericDtoBody,
  AuthLoginRequest,
  AuthRegisterRequest,
...
import { INIT_GENERIC_DTO_AUTH_lOGIN_REQUEST } from "../../../utils/init_data";
...
function Login() {
  ...
  >(INIT_GENERIC_DTO_AUTH_lOGIN_REQUEST);
  ...
  useEffect(() => {
    isRegisterSuccess == true && successNotify();

    return () => {
      isRegisterSuccess != null &&
        dispatch(removeIsRegister({ isRegisterSuccess: null }));
    };
  }, []);

  const handleLogin = async () => {
    try {
      const result = await login(genericDtoBody).unwrap();
      if (result.code === "200") navigate("/");
    } catch (error: any) {
  ...
  return (
    <>
      ...
      />
      {/* Login form */}
      <div className=" bg-gradient-to-tl from-[#d9ffde] h-screen to-[#28ffff] w-full py-16 px-4">
